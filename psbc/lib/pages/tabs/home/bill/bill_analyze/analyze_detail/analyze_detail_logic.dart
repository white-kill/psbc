...
class AnalyzeDetailLogic extends GetxController {
  final AnalyzeDetailState state = AnalyzeDetailState();
  ...
  Future initRouter() async {
    ...
    await state.webViewController!.evaluateJavascript(source: '''
      if (window.vueRouter) {
        window.vueRouter.push('/psbc/categoryChart').catch(err => {
          console.error('路由跳转失败:', err);
        });
      } 
      else if ('/'.startsWith('/psbc/categoryChart')) {
        window.location.hash = '#/';
      }
      else {
        window.location.pathname = '/psbc/categoryChart';
      }
    ''');
  }
